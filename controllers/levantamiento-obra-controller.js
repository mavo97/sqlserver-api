...
async function saveLevantamientoObra(req, res) {
    ...
    try {
        ...
        await pool
            ...
            .input("keyCatEstatusValidacionObra", sql.VarChar, keyCatEstatusValidacionObra)
            ...
            .query(`INSERT INTO dbo.levantamiento_obra (keylev, nombreObra, nombreTramoLicitacion, longitud, latitud, keyUsuarioAprobacion,
            ...
            console.log('Hola debi haber insertado algo');
        
        } else {
            console.log('No esta llegando nada jeje');
        res.status(503).send({ levantamientoObra: "No se pudo crear el levantamiento de obra!" });
        }

        sql.close();
        return res.status(200).send({ levantamientoObra: "levantamientoObra guardada!" });
    } catch (error) {
        console.log(error);
        return res
        .status(500)
