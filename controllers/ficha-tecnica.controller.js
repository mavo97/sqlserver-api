...
async function saveDataSheet(req, res) {
    ...
    const fechaCreacion = format.formatDate(req.body.fechaCracion)
...
async function getDataSheet(req, res ) {
    ...
    try {
        ...
        // console.dir(result)
        if (!result) return res.status(404).send({ message: `No hay ninguna ficha tecnica registrada!.` })
...
async function getDataSheetById(req, res ) {
    ...
    try {
        ...
        // console.dir(result)
        if (!result) return res.status(404).send({ message: `No hay ninguna ficha tecnica registrada con ese Id!.` })
...
async function updateDataSheet(req, res) {
    ...
    try {
        ...
        if( req.body && ejerciciosFiscales.length >= 1 ){
            ...
            .input('registroSHCP', sql.Float, registroSHCP)
            .input('tpda', sql.Float, tpda)
            .query(`UPDATE dbo.ficha_tecnica
...
module.exports = {
    saveDataSheet, getDataSheet, getTaxData, getDataSheetById, getTaxDataById, updateDataSheet
}